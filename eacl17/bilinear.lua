...
opt.modelBase = '/local/filespace/lr346/disco/experiments/negation/nncg-negation/eacl17/models/' .. args['train'] .. '/model_bil_mse_' .. tostring(opt.hiddenSize) .. '_' .. tostring(opt.learningRate) .. '_' .. tostring(opt.batchSize)
opt.logFile = '/local/filespace/lr346/disco/experiments/negation/nncg-negation/eacl17/models/' .. args['train'] .. '/log_bil_mse_' .. tostring(opt.hiddenSize) .. '_' .. tostring(opt.learningRate) .. '_' .. tostring(opt.batchSize)
if opt.dropout then
...
local ged1 = nn.Sequential()
local prepBil = nn.ConcatTable()
prepBil:add(nn.Narrow(2, 1, opt.embedSize))
prepBil:add(nn.Narrow(2, opt.embedSize + 1, opt.gateSize))
ged1:add(prepBil)
ged1:add(nn.Bilinear(opt.embedSize, opt.gateSize, opt.hiddenSize))
ged1:add(nn.Sigmoid())

local ged2 = nn.Sequential()
ged2:add(nn.Narrow(2, opt.embedSize + 1, opt.gateSize))

local ged3 = nn.ConcatTable()
ged3:add(ged1)
ged3:add(ged2)

local ged = nn.Sequential()
ged:add(ged3)
ged:add(nn.Bilinear(opt.hiddenSize, opt.gateSize, opt.embedSize))
-- ged:add(nn.Sigmoid())
-- ged:add(nn.Linear(opt.embedSize, opt.embedSize))
